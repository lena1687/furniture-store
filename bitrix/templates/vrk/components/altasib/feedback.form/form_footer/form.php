<div class="afbf_feedback_poles">
	<script type="text/javascript">
        ...
        <form class="form-footer" id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
            <div class="form-title">
                <?$APPLICATION->IncludeComponent(
                    ...
                    Array(
                        ...
                        "EDIT_TEMPLATE" => "",
                        "PATH" => "/include/footer/form-title.php"
                    )
                );?>
            </div>
            ...
            foreach($arResult["FIELDS"] as $key=>$arField):?>
                ...
                <div id="afbf_<?=mb_strtolower($arField["CODE"])?>" class="afbf_item_pole<?=$fieldClass?>">
                    <div class="afbf_name<?=$nameClass?>">
                        <?						?> <?if($arField["REQUIRED"]):?><span class="afbf_required_text"></span><?endif?>
                    ...
                    <?				elseif($arField["TYPE"] != "F"):
                        ...
                        <div class="afbf_inputtext_bg" id="error_<?=$arField["CODE"]?>">
                            ...
                            <?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
                                ...
                                <input placeholder="<? if($arField["CODE"] == "FIO_".$ALX) echo 'Введите ваше имя*'; if($arField["CODE"] == "EMAIL_".$ALX) echo 'Введите ваш E-mail*'; ?>" type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]))?>" class="afbf_inputtext" />
                            <?						else:?>
                                <input placeholder="<? if($arField["CODE"] == "FIO_".$ALX) echo 'Введите ваше имя*'; if($arField["CODE"] == "EMAIL_".$ALX) echo 'Введите ваш E-mail*'; ?>" type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" />
                            <?						endif;?>
...
<?				endif;?>
            ...
            <div class="afbf_submit_block test">
                <input type="submit" class="fb_close afbf_btn" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="Подписаться" />
            </div>
        </form>
